...
import wtf.speech.features.wallet.domain.utils.decodeBase58
import wtf.speech.features.wallet.domain.utils.encodeToBase58String
...
object BitcoinAddressGenerator : AddressGenerator {
    ...
    override suspend fun generateAddress(publicKey: PublicKey): Address {
        ...
        val checksum =
            sha256(sha256(preChecksumAddress)).take(4).toByteArray()
    ...
    override suspend fun validateAddress(address: Address): Boolean {
        ...
        val calculatedChecksum =
            sha256(sha256(data)).take(4).toByteArray()
